 ...
 * have exactly number_to_color red edges more than start_graph
 * 
 ...
 */
struct mygraph* generate_final_configuration(struct mygraph* start_graph, int number_to_color, int* nb_of_graphs)
{
    ...
    //a graph to store how many extra edges were colored red
    int edges_colored = 0;
    ...
    while( number_to_color > edges_colored){
        ...
        // the memory allocated for storing the next layer in original labeling with all struct data
        ...
        for (int existing_graph_number = 0; existing_graph_number < nb_of_old_graphs; existing_graph_number ++){
            ...
            if (!contains(generated_graphs[existing_graph_number].edgelist,2*edge) && contains(generated_graphs[existing_graph_number].edgelist,2*edge+1))
            {
                ...
                //store the original labeling for later use
                ...
                //perform a binary search in the list of canonical labelings to check isomorphic copies
...
{   
    ...
    bitset base_graph = singleton(vertices*(vertices-1)/2)-1; //This variable is a graph6 string with al edges that are allowed to be used
    ...
    fprintf(stderr,"number to color: %i\n",number_to_color);
    ...
    for (int i = 0; i < vertices; i++){
        for (int j =0; j<i; j++){
            ...
            if (blue_start_graph & singleton(edge)){
    ...
    struct mygraph* configurations = generate_final_configuration(&start_graph,number_to_color,&number_of_graphs);
